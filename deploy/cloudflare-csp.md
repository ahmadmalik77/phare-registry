...
**Then set:**
```
Content-Security-Policy: default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; font-src 'self'; img-src 'self' data:; connect-src 'self' https://YOUR-WORKER.workers.dev; frame-ancestors 'none'; base-uri 'self'; form-action 'none'
```
